  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8 animate-in fade-in pb-24">
      ...
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <span className="text-xs font-bold uppercase tracking-widest text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-3 py-1 rounded-full border border-blue-200 dark:border-blue-700">
            PAINEL PRINCIPAL
          </span>
          <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mt-2">
            Olá, {profile.name}! 👋
          </h1>
        ...
        <div className="flex gap-3">
          <div className="bg-white dark:bg-slate-900 px-4 py-2.5 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 min-w-[120px] transition-colors">
          ...
          <div className="bg-white dark:bg-slate-900 px-4 py-2.5 rounded-xl shadow-xs border border-slate-200 dark:border-slate-800 min-w-[110px] transition-colors">
            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Frequência</p>
            <p className="text-xs sm:text-sm font-semibold text-slate-800 dark:text-slate-200">{profile.daysPerWeek}x por Semana</p>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-6">
          ...
          {todayWorkoutDay ? (
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 p-6 sm:p-8 relative overflow-hidden transition-colors">
              <div className="absolute top-0 right-0 p-8 opacity-5 dark:opacity-10 text-slate-900 dark:text-white pointer-events-none">
              ...
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 relative z-10">
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    <span className="px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 rounded-full text-xs font-semibold uppercase">
                      {profile.location}
                    </span>
                    <span className="px-3 py-1 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800 rounded-full text-xs font-semibold uppercase">
                      {profile.experience}
                    </span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="px-8 py-4 bg-slate-900 hover:bg-slate-800 dark:bg-blue-600 dark:hover:bg-blue-500 text-white rounded-xl font-bold text-sm shadow-xl shadow-slate-900/20 dark:shadow-blue-600/20 active:scale-95 transition-all flex items-center justify-center gap-2 shrink-0 cursor-pointer"
                >
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xs border border-slate-200 dark:border-slate-800 p-6 text-center space-y-3 transition-colors">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-xl bg-blue-600 text-white font-bold text-xs hover:bg-blue-700 transition-colors shadow-md shadow-blue-600/20 cursor-pointer"
              >
                GERAR PLANO AGORA
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-slate-900 dark:bg-slate-850 rounded-2xl p-5 text-white border border-slate-800 flex flex-col justify-between shadow-xs">
              ...
              <button
                ...
                className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-xl text-xs font-semibold border border-slate-700 text-blue-400 flex items-center justify-center gap-2 transition-colors cursor-pointer"
              >
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-xs transition-colors">
                ...
                {history.slice(-3).reverse().map((session, idx) => (
                  <div key={session.id || idx} className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{session.date} • {session.durationMinutes} minutos</p>
                    </div>
            ...
            <button
              ...
              className="w-full mt-6 py-3 text-xs font-bold text-blue-600 dark:text-blue-400 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 border border-blue-200 dark:border-blue-800 rounded-xl transition-colors cursor-pointer"
            >
      ...
      {/* Footer info */}
      <footer className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 text-[10px] text-slate-400 uppercase tracking-widest font-bold transition-colors">
        <div className="flex flex-wrap items-center gap-6">
          <span>© 2026 TREINO IA PRO</span>
          <span className="text-red-500/80">Atenção: Consulte um profissional de saúde</span>
        </div>
        <div className="flex gap-4">
          <a href="#" onClick={(e) => e.preventDefault()} className="hover:text-slate-600 dark:hover:text-slate-300">Privacidade</a>
          <a href="#" onClick={(e) => e.preventDefault()} className="hover:text-slate-600 dark:hover:text-slate-300">Suporte</a>
        </div>
      </footer>
    </div>
  );
